...
def test_horseman_attack(target_x, target_y, expected):
    player1 = Player(1, "Player 1")
    player2 = Player(2, "Player 2")
    horseman = Horseman(player1, x=10, y=10)
    target = Archer(player2, x=target_x, y=target_y)
    game_map = BattleMap(units=[horseman, target])
    controller = GameController(game_map)
    assert controller.can_attack_unit(horseman, target) == expected